...
(
  ...
  AND (
    S3.status <> '空' OR S3.line_id <> S1.line_id
  )
);
...
FROM (
  SELECT MM.deal_date,
  COUNT(NN.row_num) - MIN(MM.row_num) AS gap
  FROM MyStockUpSeq MM INNER JOIN MyStockUpSeq NN
  ON NN.row_num <= MM.row_num
  GROUP BY MM.deal_date
) TMP
GROUP BY gap;